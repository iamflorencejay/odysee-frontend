...
export default function LivestreamPage(props: Props) {
  ...
  React.useEffect(() => {
    if (claimId) doCommentSocketConnect(claimId);

    return () => {
      if (claimId) doCommentSocketDisconnect(claimId);
    };
  }, [claimId, doCommentSocketConnect, doCommentSocketDisconnect]);

  // Find out current channels status + active live claim.
  React.useEffect(() => {
    doFetchChannelLiveStatus(channelClaimId);
    const intervalId = setInterval(() => doFetchChannelLiveStatus(channelClaimId), 30000);
    return () => clearInterval(intervalId);
  }, [channelClaimId, doFetchChannelLiveStatus]);
